...
namespace DauGia.Models
{
    public class SanPhamDAO
    {
        ...
        {
            var query = dg.Multimedias.Join(dg.SanPham_Multimedias.Where(c => c.MaSanPham == maSanPham),
        ...
        public static List<SanPham> LayDSSanPhamTheoTrang(int soTrang)
        {
            int end = soTrang * 9;
            int start = end - 9;
            var query = dg.SanPhams.Select(sp => sp).Skip(start).Take(end - soTrang + 1);
            return query.ToList();
        }
        ...
        public static List<SanPham> TimSanPhamDauGiaNhieuNhat()
        {
            ...
            {
                ds.Add(TimSanPhamTheoMaSanPham(c.MaSanPham.Value));
