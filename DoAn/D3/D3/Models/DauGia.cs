...
namespace D3.Models
{
    partial class DauGiaDataContext
    {
        ...
        {
            var query = ChiTietDauGias.GroupBy(chitiet => new { chitiet.MaSanPham }).Select(row => new { row.Key.MaSanPham, SoLuong = row.Count() }).OrderByDescending(nhieunhat => nhieunhat.SoLuong).Take(9);
